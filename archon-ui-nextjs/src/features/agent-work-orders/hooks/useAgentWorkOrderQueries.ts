 ...
 * Handles data fetching, caching, and optimistic updates for work orders.
 */
...
export function useStepHistory(workOrderId: string | undefined) {
  return useQuery<StepHistory, Error>({
    queryKey: workOrderId ? agentWorkOrderKeys.stepHistory(workOrderId) : ["disabled"],
    queryFn: () =>
...
export function useWorkOrderLogs(
  ...
  return useQuery<WorkOrderLogsResponse, Error>({
    queryKey: workOrderId ? [...agentWorkOrderKeys.logs(workOrderId), options] : ["disabled"],
    queryFn: () =>
      workOrderId
        ? agentWorkOrdersService.getWorkOrderLogs(workOrderId, options)
        : Promise.reject(new Error("No ID provided")),
    enabled: !!workOrderId,
    staleTime: STALE_TIME_NORMAL, // 30 seconds cache for historical logs
  });
}
