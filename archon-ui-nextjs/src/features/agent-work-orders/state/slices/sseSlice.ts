...
import { agentWorkOrdersService } from "../../services";
 ...
 *
 * Persisted: Partially (liveLogs and liveProgress survive HMR, connections do not)
 *
 * Pattern:
...
export const createSSESlice: StateCreator<SSESlice, [], [], SSESlice> = (set, get) => ({
  ...
  connectToLogs: (workOrderId) => {
    ...
    // Create EventSource for log stream using service method
    const url = agentWorkOrdersService.getLogsStreamUrl(workOrderId);
