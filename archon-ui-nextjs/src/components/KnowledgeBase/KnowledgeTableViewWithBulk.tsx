...
interface KnowledgeTableViewWithBulkProps {
...
export default function KnowledgeTableViewWithBulk({
  sources,
  onView,
  onEdit,
  onDelete,
  ...
  onBulkDelete,
  searchTerm,
}: KnowledgeTableViewWithBulkProps) {
  ...
  const handleSelectAll = () => {
    if (selectedSourceIds.size === sortedSources.length) {
      // Deselect all
      setSelectedSourceIds(new Set());
    } else {
      // Select all
      setSelectedSourceIds(
        new Set(sortedSources.map((s) => s.source_id))
      );
    }
  };

  const handleSelectOne = (sourceId: string) => {
  ...
  const handleBulkRecrawl = async () => {
    if (!onBulkRecrawl || selectedSourceIds.size === 0) return;

    setIsProcessing(true);
    try {
      const selectedSources = sortedSources.filter((s) =>
        selectedSourceIds.has(s.source_id)
      );
      await onBulkRecrawl(selectedSources);
      setSelectedSourceIds(new Set()); // Clear selection after success
    } finally {
  ...
  const handleBulkDelete = async () => {
    if (!onBulkDelete || selectedSourceIds.size === 0) return;
  ...
  const allSelected =
    ...
    selectedSourceIds.size === sortedSources.length;
  const someSelected = selectedSourceIds.size > 0;
            ...
            {shouldShowEmptyState ? (
              ...
              <>
                {sortedSources.map((source) => {
                  ...
                  const isSelected = selectedSourceIds.has(source.source_id);
                    ...
                    >
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => handleSelectOne(source.source_id)}
                          className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600 cursor-pointer"
                      ...
                      {/* Title */}
                      <td
                        className="px-4 py-3 font-medium text-gray-900 dark:text-white cursor-pointer"
